...
type CalendarEventsStrings = keyof typeof CalendarEvents;
export type ICalendarHandlersMap = {
  [key in CalendarEventsStrings]: (...args: never[]) => never;
} & {
  [CalendarEvents.change]: (
    date: Date,
    oldDate: Date,
    byClick: boolean
  ) => void;
  [CalendarEvents.beforeChange]: (
    date: Date,
    oldDate: Date,
    byClick: boolean
  ) => boolean | void;
  ...
  [CalendarEvents.dateHover]: (date: Date, e: MouseEvent) => never;
};
